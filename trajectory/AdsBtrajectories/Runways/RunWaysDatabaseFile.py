...
from Runways.RunWayFile import RunWay
...
class RunWayDataBase(object):
    ...
    def read(self):
        ''' this method now reads the whole file - not only the headers '''
        ...
        for row in range(self.sheet.nrows): 
            ...
            if row == 0:
                ...
                self.runwaysArray.append ( self.getInternalRunWays(rowValues) )
    ...
    def getNumberOfRunways(self, airportICAOcode):
        ...
        assert not(self.sheet is None)
        count = 0
        for runwayDict in self.runwaysArray:
            #print ( runwayDict )
            if runwayDict:
                ''' check if dict is empty '''
                for runway in runwayDict.values():
                    if ( runway.getAirportICAOcode() == airportICAOcode):
                        count = count + 1
                        #print ( runway )
