...
from utils import readAircraftOpenapData , readOpenapEngines
...
def computeDefaultEngineFuelFlowKg(  row ):
    ''' comute global fuel flow consumption over the whole flight duration '''
    flight_duration_minutes = row['flight_duration']
    flight_duration_seconds = flight_duration_minutes * 60.0
    number_of_engines = row['number_of_engines']
    fuel_flow_kg_seconds = row['fuel_flow_kg_sec']
    return number_of_engines * flight_duration_seconds * fuel_flow_kg_seconds
...
if __name__ == '__main__':
    ...
    df_openap_engines = readOpenapEngines()
    df = df.merge( df_openap_engines, how="left", on="flight_id" )
    print ( df.shape )
    print ( list ( df ))
    ...
    print ("-"*80)
    print("--- compute average cumulated fuel flow kg over teh whole flight duration ---")
    df['fuel_flow_kg'] = df.apply( lambda row : computeDefaultEngineFuelFlowKg ( row  ) , axis=1)


    ColumnsToKeep = ['flight_id','aircraft_type','fuel_flow_kg', 'flown_distance','flight_duration']
    ...
    outputCsvFileName = "extendedOpenap_FuelFlow.csv"
