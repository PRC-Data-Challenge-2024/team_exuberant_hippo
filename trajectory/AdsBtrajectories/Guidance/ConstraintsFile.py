...
from .WayPointFile  import WayPoint 
...
def mayBeKnotsSpeedConstraint(fixIndex, fix):
    ...
    if str(speedKnots).isdigit():
        ...
        logging.info ( 'Constraints: speed constraint expressed as Knots= {0}'.format(speedKnots) )
        ...
        if indexOfLevelConstraint >= 0:
            ...
            if str(speedKnots).isdigit():
                logging.info ( 'Constraints: speed constraint= {0} knots'.format(speedKnots) )
                speedConstraint = SpeedConstraint(fixIndex = fixIndex , 
                                                          speed = speedKnots ,
                                                          units = 'knots')
                constraintFound = True
                levelFlightLevel = str(subString[indexOfLevelConstraint:])
                logging.info ( 'Constraints: level constraint= {0}'.format(levelFlightLevel) )
            ...
            if indexOfLevelConstraint >= 0:
                ...
                if str(speedKnots).isdigit():
                    ...
                    logging.info ( 'Constraints: level constraint= {0}'.format(levelAltitudeFeet) )
                else:
...
def mayBeMachSpeedConstraint(fixIndex, fix):
    ...
    if str(speedMach).isdigit():
        ...
        logging.info ( 'Constraints: speed constraint expressed as Mach= {0}'.format(speedMach) )
        speedConstraint = SpeedConstraint ( fixIndex = fixIndex,
                                                    speed = speedMach,
                                                    units = 'mach')
        constraintFound = True
...
class Constraints(object):
    ...
    def __init__(self, fixIndex ):
        ...
        ''' after take-off '''
        if fixIndex == 0:
            logging.info ( self.className + ' constraint is applicable after Take Off or first fix' )
...
class SpeedConstraint(Constraints):
    ...
    def __init__(self, fixIndex, speed , units ):
        ...
        if units == 'mach':
            ...
            logging.info ( self.className + ': speed constraints after fixIndex= {0} - speed= {1} - units= {2}'.format(self.fixIndex,
...
class LevelConstraint(Constraints):
    ...
    def __init__(self, fixIndex, level , units ):
        ...
        logging.info ( self.className + ': level constraints after fixIndex= {0} - level= {1} meters - level= {2} feet'.format(self.fixIndex,
